...
MigR1_4 <- `4th_MigR1`
tmp <- list()
tmp <- c("AE_1","MigR1_1","AE_2","MigR1_2","AE_3","MigR1_3","AE_4","MigR1_4")
...
filter_cell = 5
metadata <- data.frame(
  matrix(unlist(strsplit(colnames(counts),"-")), ncol = 2, byrow = T),
  row.names = colnames(counts)
)
colnames(metadata) <- c("tech","UMI")
metadata$celltype <- "undefined"
...
metadata <- data.frame(
  matrix(unlist(strsplit(colnames(dge),"-")), ncol = 2, byrow = T),
  row.names = colnames(dge)
)
colnames(metadata) <- c("tech","UMI")
metadata$celltype <- "undefined"
message("Metadata done")
##########构建seurat对象###############
...
plot1 <- FeatureScatter(pbmc, feature1 = "nCount_RNA", feature2 = "percent.mt",group.by = "tech")
plot2 <- FeatureScatter(pbmc, feature1 = "nCount_RNA", feature2 = "nFeature_RNA",group.by = "tech")
plot1 + plot2
